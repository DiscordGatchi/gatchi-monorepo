...
const handler: RouteHandlerMethod = async (request, reply) => {
  ...
  const secrets = {
    endpoint: await getSecret('AWS_ENDPOINT'),
    bucket: await getSecret('AWS_BUCKET'),
  ...
  const customDomain = await getSecret('CUSTOM_DOMAIN')
  const responseUrlBase = customDomain
    ? customDomain
